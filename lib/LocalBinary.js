...
function LocalBinary(){
  ...
  this.downloadSync = function(conf, destParentDir, retries) {
    ...
    try{
      const userAgent = [packageName, this.version].join('/');
      const obj = childProcess.spawnSync(cmd, opts, { env: { 'USER_AGENT': userAgent } });
      let output;
  ...
  this.download = function(conf, destParentDir, callback, retries){
    ...
    options.headers = Object.assign({}, options.headers, {
      ...
      'user-agent': [packageName, this.version].join('/'),
    });
